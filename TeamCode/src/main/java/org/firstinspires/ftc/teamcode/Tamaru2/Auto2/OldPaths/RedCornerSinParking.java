...
/**
 * Class name: RedCornerSinParking
 * Class Type: auto
 * Class Function: park in red corner
 * Other Notes: uses sinDrive function and color sensor to park in red corner
 */

@Autonomous
@Disabled

public class RedCornerSinParking extends AutoBase2{
    ...
    public void runOpMode(){
        ...
        if(senseColorsFront().equals("green")){
            sinDrive(0, -12, 0, .35, .5, .3, 2, 2, 1, 2);
            sinDrive(-25, 0, 0, .75, .3, .3, 2.5, 2, 2, 2);
            sinDrive(0, -12, 0, .35, .5, .3, 2, 2, 1, 2);
            telemetry.addData("parked", "green");
            telemetry.update();
        }else if(senseColorsFront().equals("blue")){
            sinDrive(0, -14, 0, .55, .75, .35, .25, 1, 1, 2);
            sinDrive(2, 0, 0, .35, .35, .75, 2, 2, 1, 2);
            telemetry.addData("parked", "blue");
            telemetry.update();
        }else{
            sinDrive(0, -10, 0, .35, .75, .35, 2, 2, 1, 2);
            sinDrive(40, 0, 0, 1, .35, .35, 2, 2, 1, 2);
            telemetry.addData("parked", "red");
            telemetry.update();
        }

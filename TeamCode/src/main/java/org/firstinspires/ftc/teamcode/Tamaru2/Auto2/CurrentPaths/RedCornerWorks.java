...
/**
 * this is before PID was retuned, works consistently if voltage is about 12.4-12.9, but gets screwed up if outside of that range
 */

@Config
@Autonomous
@Disabled
public class RedCornerWorks extends LinearOpMode {
    ...
    public void runOpMode(){
        ...
        while(opModeIsActive()) {
            ...
            Control(30, 0, 0, midPoleArmTarget, robot.turretForward, 0, robot.handClosed, 1.5, false);
            Control(62, 0, 0, midPoleArmTarget, robot.turretForward, 0, robot.handClosed, 1.5, false);
            ...
            Control(40, 0, 0, midPoleArmTarget, robot.turretForward, 0, robot.handClosed, 1, true);
            //set extension and turret over pole
            robot.servoExtend.setPosition(.25);//.125
            ...
            Control(56, 0, 0, fiveConeArmTarget, robot.turretForward, 0, robot.handOpen, 1.5, false);
            PIDDriveControl(56, 0, -90, 1.5, false);
            PIDDriveControl(83, 0, -90, 1.5, false);
            //grab top cone
            robot.servoHand.setPosition((robot.handClosed));
            sleep(500);
            PIDArmControl(lowPoleArmTarget, 1);
            //score top cone
            PIDDriveControl(66.5, 0, -90, 1.5, true);//67.5
            ...
            PIDDriveControl(83, 0, -90, 1, false);
            //grab second cone
            robot.servoHand.setPosition(robot.handClosed);
            sleep(500);
            PIDArmControl(lowPoleArmTarget, 1);
            //score second cone
            PIDDriveControl(66.5, 0, -90, 1.5, true);
            robot.servoTurret.setPosition(robot.turretPort);
            robot.servoExtend.setPosition(.5);
            sleep(1500);
            robot.servoHand.setPosition(robot.handOpen);
            ...
            robot.servoHand.setPosition(robot.handClosed);
            if(color=="green"){
                PIDDriveControl(33, 0, -90,1, false);
            }else if(color=="blue"){
                PIDDriveControl(50, 0, -90, 1, false);
            }else{
                PIDDriveControl(83, 0, -90, 1, false);
            }
    ...
    public void Control(double targetY, double targetX, double targetTheta, double targetArm, double targetTurret, double targetExtension, double targetHand, double timeout, boolean dist){
        ...
        while(((!yController.atSetPoint())||(!xController.atSetPoint())||(!thetaController.atSetPoint())||(!armController.atSetPoint()))&&(getRuntime()<timeout)) {
            ...
            if((dist==true)&&((robot.distSensorStar.getDistance(DistanceUnit.CM)<15)||(robot.distSensorPort.getDistance(DistanceUnit.CM)<15))){
                break;
            }
    ...
    public void PIDDriveControl(double targetY, double targetX, double targetTheta, double timeout, boolean dist){
        yController.setPID(py, iy, dy);
        xController.setPID(px, ix, dx);
        thetaController.setPID(pTheta, iTheta, dTheta);

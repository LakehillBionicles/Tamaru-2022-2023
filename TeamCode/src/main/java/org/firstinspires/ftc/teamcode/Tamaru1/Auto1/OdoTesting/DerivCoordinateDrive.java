...
public class DerivCoordinateDrive extends AutoBase {
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            coordinateDrive(0, 24, 24, .2, 0.2, 0.2, 2, .5, 0.5); //theta x y power power power tol tol tol

            sleep(10000);

            coordinateDrive(0, -24, -24, .2, 0.2, 0.2, 2, .5, 0.5);
    ...
    public void coordinateDrive(double thetaTarget, double xTarget, double yTarget, double rotPower, double linXPower, double linYPower, double rotTol, double xTol, double yTol){
        ...
        while (Math.abs(thetaError) > Math.toRadians(rotTol) || Math.abs(xError) > xTol || Math.abs(yError) > yTol){
            ...
            if (Math.abs(thetaError) > Math.toRadians(rotTol)){
                ...
                thetaPower = -((Kd * thetaDeriv) + (Kp * Math.signum(thetaError)));
            }
            else{
                thetaPower = 0;
            }
            ...
            if (Math.abs(xError) > xTol){
                ...
                xPower = ((Kd * xDeriv) + (Kp * Math.signum(xError)));
            }
            else{
                xPower = 0;
            }
            ...
            if (Math.abs(yError) > yTol){
                ...
                yPower = ((Kd * yDeriv) + (Kp * Math.signum(yError)));
            }
            else{
                yPower = 0;
            }
